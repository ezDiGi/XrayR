...
create_docker_compose(){
    ...
    cat > /etc/XrayR/docker-compose.yml <<EOF
version: '3'
services: 
  xrayr: 
...
create_dns(){
    echo -e "${green}Tạo file dns.json${plain}"
...
create_AikoBlock(){
    echo -e "${green}Tạo file AikoBlock${plain}"
    cat > /etc/XrayR/AikoBlock <<EOF
catport.vn
cloudfast.vn
zingfast.net
zingfast.vn
...
EOF
  ...
  sed -i "s|DeviceLimit:.*|DeviceLimit: ${limit}|" /etc/XrayR/config.yml
  sed -i "s|SpeedLimit:.*|SpeedLimit: ${limit_speed}|" /etc/XrayR/config.yml
}
