...
pre_install_docker_compose() {
 # web
 read -p "  ApiHost (api_host):" api_host
  [ -z "${api_host}" ] && api_host=0
  echo "--------------------------------"
  echo "  Bạn đã chọn ${api_host}"
  echo "--------------------------------"
# api key
 read -p "  ApiKey (api_key):" api_key
  [ -z "${api_key}" ] && api_key=0
  echo "--------------------------------"
  echo "  Bạn đã chọn ${api_key}"
  echo "--------------------------------"
# id nút
  read -p "  ID nút (Node_ID):" node_id
  [ -z "${node_id}" ] && node_id=0
  echo "-------------------------------"
  echo -e "  Node_ID là: ${node_id}"
  echo "-------------------------------"
# giới hạn tốc độ
    read -p " Giới hạn tốc độ (Mbps):" limit_speed
    [ -z "${limit_speed}" ] && limit_speed=0
    echo "-------------------------------"
    echo -e "Giới hạn tốc độ là: ${limit_speed}"
    echo "-------------------------------"
  # giới hạn thiết bị
...
chmod +x /usr/local/bin/docker-compose
  ...
  docker-compose up -d
  echo
  echo -e "  Đã hoàn tất cài đặt phụ trợ docker ！"
